...
class PostgresMetadataCache:
    ...
    async def generate_metadata(self) -> dict:
        ...
        table_rows = await self.conn.fetch("""
        ...
        for row in table_rows:
            ...
            # 3.1: Column schema
            schema_rows = await self.conn.fetch("""
            ...
            for col in columns:
                try:
                    distinct = await self.conn.fetch(
            ...
            # 3.3: Example row
            try:
                example = await self.conn.fetchrow(f"SELECT * FROM {table} LIMIT 1")
